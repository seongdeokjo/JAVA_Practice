...
--update 테이블 명 set 컬럼이름 = 새로운 데이터, 컬럼이름 = 새로운 데이터
--              where 행을 선택하는 조건 
...
;
--where 
--10번 부서의 위치를 busan 으로 이동, 부서이름 DEV 변경 
update dept03
...
--scott 사원의 부서번호는 20번으로, 직급은 MANAGER로 한꺼번에 수정하도록 하자
update emp01
set deptno = 30 , job = 'MANAGER'
where ename = 'SCOTT'
;
--입사일자는 오늘로, 급여를 50으로 커미션을 4000으로 수정
update emp01
...
--부서번호가 10번인 부서와 동일하게 변경하도록 해 봅시다.
...
--사원 테이블에서 부서명이 SALES인 사원을 모두 삭제하자.
