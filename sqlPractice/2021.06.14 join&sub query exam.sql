...
select  bookname
from book
where bookid not in (select o.bookid from orders o, customer c where o.custid = c.custid and c.name = '박지성')
;
...
--(10) 고객의이름과고객별구매액
select (select c.name from customer c where c.custid = o.custid ) as name ,sum(o.saleprice)
from orders o
group by o.custid;
