...
select empno,ename,job,sal, decode(job, 'ANALYST' , sal + 200,
                            'SALESMAN', sal + 180,
                            'MANAGER', sal + 150,
                            'CLERK', sal +100) as jobsal
from emp
order by job desc;

--23. 모든 사원의 급여 최고액, 최저액, 총액 및 평균 급여를 출력하시오. 
--    평균에 대해서는 정수로 반올림하시오.
select max(sal) as "급여 최고액",min(sal) as "급여 최저액", sum(sal) as "급여 총액",
...
select job, max(sal) as "담당 급여 최고액", min(sal) as "담당 급여 최소액", sum(sal) as "담당 급여 총액",
    round(avg(sal)) as "담당 업무 평균"
from emp
group by job
order by job desc;
...
--    관리자를 알 수 없는 사원과 최저 급여가 2000 미만인 그룹은 제외시키고 결과를 급여에 대한
--    내림차순으로 정렬하여 출력하시오.
